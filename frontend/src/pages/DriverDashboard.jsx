...
export default function DriverDashboard({ onOpenVerification }) {
  ...
  const handleConfirmPayment = async (booking) => {
    const paymentMethod = window.prompt(`Confirm payment receipt from ${booking.passenger_name} (LKR ${(booking.price_per_seat || 300) * (booking.seats_booked || 1)}). Enter payment method:`, 'Cash Payment');
    if (paymentMethod === null) return; // Driver clicked Cancel in prompt
    ...
    try {
      const res = await fetch(`/api/bookings/${booking.id}/confirm-payment`, {
        ...
        body: JSON.stringify({ paymentMethod: paymentMethod || 'Cash Payment' })
      });
      ...
      if (res.ok) {
        ...
        addToast?.({
          ...
          message: `Payment from ${booking.passenger_name} verified. Digital receipt sent to passenger.`,
          duration: 5000
        });
      } else {
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-white/[0.06]">
                    ...
                    {b.status === 'confirmed' && (
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-black rounded-xl transition shadow-md shadow-emerald-600/30 flex items-center gap-1.5"
                      >
                        <CreditCard className="w-3.5 h-3.5" />
                        <span>Confirm Payment & Drop-Off</span>
                      </button>
                    )}
                    ...
                    {b.status === 'completed' && (
                      <span className="text-[11px] font-bold text-emerald-400 bg-emerald-500/15 border border-emerald-500/30 px-2.5 py-1 rounded-xl flex items-center gap-1">
                        <Check className="w-3.5 h-3.5" /> Payment Received (LKR {b.paid_amount || (b.price_per_seat * b.seats_booked)})
                      </span>
                    )}
                  </div>
                </div>
