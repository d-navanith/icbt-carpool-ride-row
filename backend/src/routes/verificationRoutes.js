...
const isValidUrlOrEmpty = (
  ...
  if (
    typeof value !== "string" ||
    value.length > MAX_DOCUMENT_URL_LENGTH
  ) {
    return false;
  }
  ...
  (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Vehicle seat count must be between 1 and 12.",
        });
      }
      ...
      const safeLicenseDoc =
        license_doc_url?.trim() ||
        defaultLicenseDoc;

      const safeVehiclePhoto =
      ...
      return res
        ...
        .json({
          error:
            error.message ||
            "Server error while submitting driver verification.",
        });
    }
  },
);
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      console.error(
        "Driver verification status error:",
        error,
      );
  ...
  (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Valid Student/Staff ID card number is required.",
        });
      }
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "ID document URL must be a valid HTTP/HTTPS URL.",
        });
      }
      ...
      return res
        ...
        .json({
          error:
            error.message ||
            "Failed to submit passenger verification.",
        });
    }
  },
);
