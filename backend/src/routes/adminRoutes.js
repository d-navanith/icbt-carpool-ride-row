...
const assertCurrentAdminPassword = (
  adminId,
  currentPassword,
) => {
  if (
...
router.get("/stats", (req, res) => {
  try {
    ...
    const pendingDriverVerifications = db
      .prepare(
        `
        SELECT COUNT(*) as count
        FROM driver_verifications
    ...
    const oddDrivers = db
      .prepare(
        ...
        SELECT COUNT(*) as count
        FROM driver_verifications
        WHERE odd_even_type = 'ODD'
        AND status = 'approved'
        `,
      )
      .get().count;

    const evenDrivers = db
      .prepare(
        `
        SELECT COUNT(*) as count
        FROM driver_verifications
        WHERE odd_even_type = 'EVEN'
        AND status = 'approved'
        `,
      )
      .get().count;

    const estimatedLitersSaved =
      (totalBookings * 3.2).toFixed(1);

    const estimatedCostSavedLKR =
      (
        estimatedLitersSaved * 370
      ).toLocaleString();
  ...
  try {
    ...
    return res
      ...
      .json({
        error:
          error.message ||
          "Failed to fetch administrator account.",
      });
  }
});
...
router.put(
  ...
  (req, res) => {
    try {
      ...
      const studentStaffId =
        req.body.student_staff_id ===
        undefined
      ...
      if (!name) {
        return res.status(400).json({
          error:
            "Administrator name is required and must be 100 characters or fewer.",
        });
      }
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Phone and staff ID must be valid text values.",
        });
      }
...
router.post(
  ...
  (req, res) => {
    try {
      ...
      return res.json({
        message:
          "Administrator password changed successfully. Please sign in again with the new password.",
      });
    } catch (error) {
      console.error(
  ...
  (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error: `New administrator password must be at least ${MIN_PASSWORD_LENGTH} characters.`,
        });
      }
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Staff ID and phone must be valid text values.",
        });
      }
...
// =========================================================
// 7. Promote Existing User to Administrator
// =========================================================
...
// =========================================================
// 8. Get All Driver Verifications Queue
// =========================================================
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      return res.status(500).json({
        error:
          "Failed to fetch verifications queue.",
      });
    }
  },
);
...
// =========================================================
// 9. Driver Verification Action
// =========================================================

router.post(
  "/verifications/driver/:id/action",
  (req, res) => {
    try {
      const {
        action,
...
// =========================================================
// 10. Passenger Verification Action
// =========================================================

router.post(
  "/verifications/passenger/:id/action",
  (req, res) => {
    try {
      const {
        action,
      ...
      return res.json({
        message: `Passenger verification updated to ${action}.`,
      });
    } catch (error) {
      return res.status(500).json({
        error:
          "Failed to update passenger verification.",
      });
    }
  },
);
...
// =========================================================
// 13. Delete / Cancel Ride as Admin
// =========================================================
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="icbt_campus_rides.csv"',
      );
      ...
      console.error(
        "Export rides CSV error:",
        error,
      );

      return res.status(500).json({
        error:
          "Failed to export rides CSV.",
      });
    }
  },
);
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="icbt_campus_users.csv"',
      );
